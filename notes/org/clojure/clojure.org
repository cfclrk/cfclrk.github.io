...
* Tap
  ...
  #+begin_src clojure
    (def tap-atom (atom []))
    (add-tap (fn [x] (swap! tap-atom conj x)))
  #+end_src
...
** neil
   ...
   On GitHub: [[https://github.com/babashka/neil][neil]]

   #+header: :dir ~/Projects/codenotes/clojure
   #+begin_src sh
     neil new --name org-clojure-demo
   #+end_src

   #+RESULTS:
   : Creating project from org.corfield.new/scratch in org-clojure-demo
...
** Read and write a binary file
   ...
   #+begin_src clojure
     ...
     (save-file-bytes mypdf "/Users/cclark/cats/test.pdf")
   #+end_src
