...
** list-deployments
   ...
   #+begin_src sh
     aws deploy list-deployments \
         --application-name "e9y" --deployment-group-name prod-environment
   #+end_src
...
** get-deployment
   ...
   #+begin_src sh
     aws deploy get-deployment \
         --deployment-id "" \
         | jq '.deploymentInfo | {createTime, completeTime}'
   #+end_src
...
** list-endpoints
   ...
   #+begin_src sh
     aws sagemaker list-endpoints | jq '.Endpoints[].EndpointName'
   #+end_src
...
** invoke
   ...
   #+begin_src sh
     aws sagemaker-runtime invoke-endpoint \
         ...
         --body fileb:///Users/cclark/example.json \
...
** start-message-move-task (DLQ redrive)

   Redrive an SQS message from a DLQ back to its source queue.
