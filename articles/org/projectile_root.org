...
Instead: when I'm working in, say, a python project that happens to be in a
monorepo, I want Projectile to operate on that python project. One could still
use Projectile on the whole monorepo by navigating to a file at the root of the
monorepo.
...
** Marker Files
   ...
   I also add =requirements.txt=, which is a useful way for me to demarcate a
   small Python example (like a GitHub Gist) that maybe doesn't warrant a whole
   =setup.py=, and some files for the Bazel build system.
