...
* Example of the Problem
  ...
  #+header: :var NotesProject=NotesProject
  ...
  I /want/ projectile to say that the project root is
  =/private/tmp/notes/go/projectA=. By default, however, projectile determines
  the monorepo root directory is the project root.

  To understand how to customize projectile to say the project root is
  =/private/tmp/notes/go/projectA=, we first need to understand how projectile
  detects projects.
...
* Project Detection
  ...
  if we wanted =/private/tmp/= to be the project root for some reason, we could
...
** Marker Files
   ...
   So, we just need to add =setup.py= and =go.mod= to
   =projectile-project-root-files-bottom-up=. But doesn't projectile already know
...
** Python
   ...
   #+begin_src emacs-lisp :var NotesProject=NotesProject
     (let ((default-directory
             (f-join NotesProject "python/projectC")))
       (projectile-project-root))
   #+end_src
