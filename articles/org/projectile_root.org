...
* The Problem
  ...
  When I'm in a python project that happens to be in a monorepo, I still want
  projectile to operate on the python project. I should still be able to operate
  on the monorepo by opening a file or Dired buffer at the root of the monorepo.

  To jump to the solution, go to [[#solution]], or see the generated elisp file I
  use [[https://github.com/cfclrk/dotfiles/blob/master/emacs/projectile-discovery.el][on GitHub]].
...
* Project Detection
  ...
  To understand why this is, we can look at the source of that function.
  ...
  To illustrate the two takeaways above, say we append =go.mod= to that list of
  marker files:
  ...
  Projectile found a =.git= two directories up before looking for a =go.mod=. We
  could rectify this by /prepending/ =go.mod= instead of appending it, but the
  general problem would still be there.

  This behavior is perhaps useful in one situation: if you want to force a
  project root to a /higher/ level. For example, if we wanted =/tmp/= to be the
  project root for some reason, we could just put a =.projectile= there.
...
* Project Detection Issues
  ...
  2. *Lookup Order*: Even if projectile /were/ looking for =setup.py= and
     =go.mod=, the ordering of the list matters. Projectile might find some
     other marker file at a higher level before looking for =go.mod= or
     =setup.py=.
...
** Marker Files
   ...
   project root. Projectile already has a variable for this, documented in [[https://docs.projectile.mx/projectile/projects.html#file-markers][File
   markers]]:
...
** Lookup Order
   ...
   Instead, we want to look for for /every/ marker file in that directory before
   continuing to the parent directory.

   Instead of looping over the marker files and running =locate-dominating-file=
   on each marker file, we should loop over the /directories/ and run a function
   that checks whether any maker file is in that directory.

   Luckily, the [[https://github.com/rejeep/f.el][f]] library already has a perfect function for that already:
   [[https://github.com/rejeep/f.el#f-traverse-upwards-fn-optional-path][f-traverse-upwards]].
...
* Solution
  ...
  1. Define a variable with the marker files that indicate a project root.
  ...
  2. Define a new function for discovering projects.

     #+header: :tangle (expand-file-name "~/emacs/projectile-discovery.el")
     #+begin_src emacs-lisp :results silent
       (defun any-file-in-dir? (files dir)
         "True if any of FILES are in DIR.
       FILES is a list of file names. DIR is a directory name."
         (cl-some
     ...
     #+begin_src emacs-lisp :results silent
       ...
       Files in lower directories have higher precedence. If LIST is
       nil, use `my-project-root-files' instead. Return the
  ...
  3. Insert our new lookup function into =projectile-project-root-functions=.
  ...
  4. Restrict projectile's original bottom-up discovery to only work for
     =.projectile= files.
...
** Force Root Higher
   ...
   Can we force projectile to discover the monorepo instead of a subproject?
