...
* Project Detection
  ...
  Detection]]. The specific relevant function is =projectile-root-bottom-up=; that
  is what is determining the monorepo to be the project root directory.

  We can see it in action:

#+header: :prologue rm /tmp/.projectile
  #+begin_src emacs-lisp
  ...
  The source of that function is:
