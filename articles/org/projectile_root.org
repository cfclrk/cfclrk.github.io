...
1. A single git repository to hold several, self-contained projects.
   ...
   Such projects could just as easily be separate repos, but are dumped together
   into one repo for organizational purposes. The motivation here is to stave
   off an explosion of small repositories, and to group projects under a single
   team where they can more quickly see all issues and PRs.

2. A single git repository with common libraries and multiple indenpently
   deplayable services.

   This is what "monorepos" are really supposed to be about - a single commit
