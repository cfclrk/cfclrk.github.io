...
In monorepos, [[https://docs.projectile.mx/projectile/index.html][Projectile]] determines the project root to be the /monorepo/, not
the subproject that you're in.

In this article I update Projectile to instead prioritize the most specific
project it can find.

To jump to the solution, go to [[#solution]]. Or, see the Elisp file generated from
this document [[https://github.com/cfclrk/dotfiles/blob/master/emacs/projectile-discovery.el][on GitHub]] (which I use in my personal Emacs config).
...
* Two Kinds of Monorepos
  ...
  1. A single git repository to hold several, self-contained projects.
     ...
     The motivation here is to stave off as explosion of small repositories, and
     to group projects under a single team where they can more quickly see all
     issues and PRs.
  ...
  2. A single git repository with common libraries and multiple independently
     ...
     describes a single, working version of several services that may need to work
     together and share code.

  Always in case 1, and sometimes in case 2, I would rather projectile consider a
  subproject as a "project root".

  When I'm working in, say, a python project that happens to be in a monorepo, I
  want Projectile to operate on that python project. One could still use
  Projectile on the whole monorepo by navigating to a file at the root of the
  monorepo.
...
* Example of the Problem
  ...
  But alas, Projectile instead found =/tmp/repo/=.
...
* About Projectile Project Detection
  ...
  has a few strategies for finding a project root, and tries each one in order.
  The order is defined by this variable:
  ...
  The second function -- =projectile-root-bottom-up= -- is our culprit. We can
  ...
  That behavior of the =(projectile-root-bottom-up)= function is useful in one
  situation: when you want to force a project root to a /higher/ level,
  /despite/ the presence of marker files in the current directory.

  For example: if we wanted =/tmp/= to be the project root for some reason, we
  could just put a =.projectile= file there.
  ...
  That works! Ok, we should try to retain that behavior.
...
* Project Detection Issues
  ...
  2. *Lookup Behavior*: Even if Projectile knows to look for =setup.py= and
     ...
     Projectile might find some other, totally unrelated file at a higher level
     before even looking for =go.mod= or =setup.py=.
...
* Solution
  ...
  #+begin_src emacs-lisp :exports none
    ;;; projectile-discovery.el -- My project discovery config -*- lexical-binding:
    ;;; t; -*-

    ;;; Commentary:

    ;; Update projectile's project discovery behavior to prioritize the most
    ;; specific project it can discover. In monorepos, this means projectile will
    ;; find subprojects before finding the monorepo root.

    ;; To override this behavior, manually place a `.projectile' file at a higher
    ;; level.
  ...
  _The Elisp file generated from this section is [[https://github.com/cfclrk/dotfiles/blob/master/emacs/projectile-discovery.el][on GitHub]] (which I use in my
  personal Emacs config)._
  ...
  Define a new discovery function. In keeping with how exsting projectile
  project discovery functions work, allow elements to be predicates.
  ...
  by creating a =.projectile= file in a parent directory.
