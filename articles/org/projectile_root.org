...
* Project Detection
  ...
  So, projectile starts by looking for a =.projectile= between here and =/=,
  then looks for a =.git= between here and =/=, finds a =.git= at
  =/tmp/repo/.git=, and returns =/tmp/repo=.

  To illustrate this, say we append =go.mod= to that list of marker files:
...
** Lookup Order
   ...
   Instead, we want to look for for /every/ marker file in one directory before
   continuing to the parent directory.

   Instead of looping over the marker files and running =locate-dominating-file=
   on each filename, we should loop over the /directories/ and run a function
...
* Solution
  ...
  2. Define a new discovery function.
