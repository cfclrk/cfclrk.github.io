...
* The Problem
  ...
  - Projects in =~/go/= use your *work* GitHub account
  - Projects elsewhere use your *home* GitHub account

  There is a concise solution for GitHub /repos/, but /Gists/ are harder to
  handle. In this setup, Gists in your GitHub *home* account will "just work".
  For your *work* account, each Gist URL will need to be individually added to
  your git config file. All Gists are publically readable, so I tend to use
  Gists less frequently at work.
...
* The Git Config
  ...
  - =~/.config/git/config=
    ...
    - Update the =[user]= info as appropriate.
  ...
  - =~/.config/git/work.gitconfig=
    ...
    - Update the =[user]= info as appropriate.
    - Ignore the =[github]= part, that's for [[https://github.com/magit/forge][Emacs forge]].
    - In the two url-rewriting rules (the =[url]= entries):
      - Update =IronNetCybersecurity= to be you work's GitHub org name
      - Update =chrisc-ironnet= to be your work GitHub user name
      ...
      #+begin_src gitconfig :tangle ~/.config/git/work.gitconfig
        ...
        # These "insteadOf = ..." parts must be more specific than
        # the insteadOf rules for the github-home account.
...
** Git URL Rewriting
   ...
   For example, in [[https://github.com/cfclrk/cfclrk.github.io][the git repository]] where I'm writing this article, I can
   =cat= the git config file remotes and see the /real/ remote URL:
   ...
   see the /rewritten/ form of the URL:
   ...
   Importantly, multiple =url.insteadOf= rules do *not* overwrite each other.
   All rules are loaded, and the most specific match is used. From the [[https://git-scm.com/docs/git-config#Documentation/git-config.txt-urlltbasegtinsteadOf][insteadOf]]
   documentation:
