...
** Limitations
   ...
   For other GitHub account, each gist URL will need to be individually added
...
* The git config
  ...
  1. =~/.config/git/config=
     ...
     The "global" git config file. Git will always load this config.

     #+header: :tangle ~/Projects/dotfiles/xdg_config/git/config
     #+begin_src gitconfig
  ...
  2. =~/.config/git/work.gitconfig=
     ...
     The "work" git config. This overrides some values in the global git config.
     Git only loads this file if it is executed from specific directories.
